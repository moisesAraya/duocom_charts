...
describe('node-firebird-native-api', () => {
    ...
    beforeAll(() => {
        ...
        const tempMaster = (0, lib_1.getMaster)((0, lib_1.getDefaultLibraryFilename)());
        master = (0, lib_1.getMaster)((0, lib_1.getDefaultLibraryFilename)());
        dispatcher = master.getDispatcherSync();
        // Test premature shutdown prevention. 'master' variable should still be usable.
        expect((0, lib_1.disposeMaster)(tempMaster)).toBe(true);
    });
    afterAll(() => {
        if (!master) {
            return;
        ...
        expect((0, lib_1.disposeMaster)(master)).toBe(true);
        expect((0, lib_1.disposeMaster)(master)).toBe(false);
    });
    describe('getMaster', () => {
        test('with error', () => {
            expect(() => (0, lib_1.getMaster)('not-a-firebird-client-library')).toThrow();
        });
    });
    describe('Master', () => {
        test('#getStatus()', () => {
    ...
    describe('Provider', () => {
        test('wrong status class', () => {
            expect(() => dispatcher.createDatabaseSync(master, getTempFile('wrong-class.fdb'), 0, undefined)).toThrow();
        });
        ...
        test('#createDatabaseAsync() with XpbBuilder buffer', async () => {
            const util = master.getUtilInterfaceSync();
            const status = master.getStatusSync();
            const dpbBuilder = util.getXpbBuilderSync(status, lib_2.XpbBuilder.DPB, undefined, 0);
            try {
                ...
                const attachment = (await dispatcher.createDatabaseAsync(status, getTempFile('Attachment-createDatabase-xpb.fdb'), length, buffer));
                await attachment.dropDatabaseAsync(status);
            }
            finally {
